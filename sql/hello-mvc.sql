...
create table board (
   ...
   renamed_filename varchar2(255) not null, --저장파일명 (영문자,숫자로만 이루어져있음 , 인코딩이슈없애기위해)
   reg_date date default sysdate,
...
select * from board b  left  join attachment a
on b.no = a.board_no;

select no  (select ORIGINAL_FILENAME  from  attachment a where a.board_no= b.no ) from board b;
SELECT b.*, (SELECT ORIGINAL_FILENAME FROM attachment a WHERE a.board_no = b.no) original_filename
FROM board b;
